...
감정 분석의 정확도와 위로 메시지의 진정성을 높이기 위해,  
AI에게 **역할과 출력 목적을 명확히 제시하는 프롬프트 엔지니어링 전략**을 적용했습니다.  

- **역할 명확화 (Role Assignment)**  
  → Claude 모델에게 “따뜻한 말로 감정을 공감하고 위로하는 감성적인 심리상담사”라는 역할을 부여하여,  
  감정적인 공감과 위로 중심의 응답을 유도했습니다.

- **문맥 인식 강조 (Context-aware Prompting)**  
  → “사용자가 작성한 일기에는 지치고 힘든 감정이 담겨 있습니다”라는 문장을 사전에 제시함으로써,  
  AI가 상황의 정서적 배경을 충분히 고려하도록 유도했습니다.

- **다단계 출력 요구 (Multi-output Prompting)**  
  → AI가 아래 두 가지를 순서대로 수행하도록 지시했습니다:  
    1) 최대 3개의 감정을 joy, sadness, anger, fear, surprise, neutral 중에서 선택하고 각 감정에 대해 **0~100 점수화**  
    2) 사용자의 감정에 충분히 공감한 뒤, **진심 어린 위로 문장**을 2~3줄로 생성

- **위로 메시지의 어투·형식 명시**  
  → 감정적 거리감이 없는 따뜻한 문장을 생성하도록 요청하여  
  위로가 기계적으로 느껴지지 않도록 프롬프트를 정교화했습니다.

이러한 설계로써, 단순한 키워드 분석보다  
**사용자 맞춤 감정 분석과 정서적 연결감을 모두 제공하는 AI 응답**을 가능하게 했습니다.
