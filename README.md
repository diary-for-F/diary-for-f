...
- 모든 기능은 Lambda 단위로 모듈화되어 있어, 기능별 배포 및 롤백이 용이합니다.
- 각 Lambda 함수는 역할(Role)이 분리된 IAM 권한을 사용하며, RDS나 Bedrock 접근 권한은 최소한으로 설정되어 **보안 사고 발생 시 피해 범위를 최소화하는 Zero Trust 설계 원칙**을 반영했습니다.
...
이 서비스는 사용자가 작성한 일기 내용을 분석하기 위해 **AWS Bedrock의 Claude 모델**을 사용합니다.  
AI는 일기 속 단어와 문장의 맥락을 이해하고,  
여섯 가지 고정 감정인 **Joy(기쁨), Sadness(슬픔), Anger(분노), Fear(두려움), Surprise(놀람), Neutral(평온)** 중  
가장 뚜렷하게 드러나는 감정을 판단합니다.

---

사용자가 입력한 감정과 AI가 예측한 감정을 비교 분석하여,  
**감정 일치도**와 함께 적절한 **위로 메시지**를 제공합니다.  
